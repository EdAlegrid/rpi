 ...
 * Reboot your system.
 *
 * This will disable GPIO interrupts avoiding system crashes with newer kernels when using edge interrupts.
 * 
 */
	...
	gpio_config(inputPin1, 0);
  	...
  	while(1) {
		...
		/* press inputPin2 switch to turn OFF LED output */
        	...
        	/* status LED will continously blink indicating the event loop is running */
    		mswait(300);
